...
### Wait! What is going on with `geom_histogram()`, `geom_density()` and `geom_bar()`?  

```{r}
g1 <- ggplot(data = gapminder, aes(x = gdpPercap)) + geom_histogram() 
g2 <- ggplot(data = gapminder, aes(x = gdpPercap)) + geom_density() 
g3 <- ggplot(data = gapminder, aes(x = continent)) + geom_bar() 
```

```{r echo = F, fig.width = 9, fig.height= 3, out.width = "100%"}

cowplot::plot_grid(g1, g2, g3, ncol = 3, labels = c("g1", "g2", "g3"))

```

Where does the y-axis variable come from? 

---

### Some of the `geom_` functions will transform your data, creting a new variable before visualising it ...
...
```{r out.width = "90%", fig.height = 3, fig.width = 10}
