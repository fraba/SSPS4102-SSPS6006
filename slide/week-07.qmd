...
**Fitted model:**
$$\text{kid_score} = 26 + 6 \times \text{mom_hs} + 0.6 \times \text{mom_iq} + \text{error}$$
```{r}
...
fit <- lm(weight ~ c_height + male + eth, data = earnings)
```
...
**Fitted model:**
$$\text{kid_score} = -11 + 51 \times \text{mom_hs} + 1.1 \times \text{mom_iq} - 0.5 \times \text{mom_hs} \times \text{mom_iq}$$
...
$$\text{kid_score} = -11 + 1.1 \times \text{mom_iq}$$

Slope = **1.1**

### For mom_hs = 1 (completed high school)

$$\text{kid_score} = 40 + 0.6 \times \text{mom_iq}$$
