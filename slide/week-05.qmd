...
## Feedback on Student Feedback Survey
...
### 📋 Head to Canvas and complete A1 (Section 1: Data Cleaning Workflow)
...
### 📋 Head to Canvas and complete A1 (Section 2: Data Validation and Testing)
...
For continuous variables where all outcomes within a range are equally likely. 
...
p_norm <- tibble(x = rnorm(n, mean = 0, sd = 1)) |>
  ggplot(aes(x)) +
  geom_histogram(fill = "#E64626", alpha = 0.6) +
  labs(title = "Normal (μ = 0, σ = 1)") +
  theme_minimal(base_size = 14)

p_unif <- tibble(x = runif(n, min = 0, max = 1)) |>
  ggplot(aes(x)) +
  geom_histogram(fill = "#0148A4", alpha = 0.6) +
  labs(title = "Uniform (min = 0, max = 1)") +
  theme_minimal(base_size = 14)

p_binom <- tibble(x = rbinom(n, size = 20, prob = 0.5)) |>
  ggplot(aes(x)) +
  geom_histogram(fill = "#00828A", alpha = 0.6) +
  labs(title = "Binomial (n = 20, p = 0.5)") +
  theme_minimal(base_size = 14)

p_pois <- tibble(x = rpois(n, lambda = 4)) |>
  ggplot(aes(x)) +
  geom_histogram(fill = "#F49E17", alpha = 0.6) +
  labs(title = "Poisson (λ = 4)") +
  theme_minimal(base_size = 14)

(p_norm + p_unif) / (p_binom + p_pois)
```

::: notes
Each plot shows a kernel density estimate from 10,000 random draws. Note that the normal and uniform are continuous distributions, while the binomial and Poisson are discrete — `geom_density()` smooths the discrete values using kernel density estimation, which is why they appear as smooth curves rather than bars. You could alternatively use `geom_bar()` for the discrete cases.
:::
...
### 📋 Head to Canvas and complete A1 (Section 3: Probability Distributions)
...
### 📋 Head to Canvas and complete A1 (Section 4: Simulation in R)
